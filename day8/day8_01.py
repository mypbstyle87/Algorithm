...
# 개당 순익 설정
benefit=input_list[2]-input_list[1]

# 이익이 없거나 0이면 손익분기점은 없습니다.(최소한 이익이 0보다 커야, 고정비를 커버해서 손익분기 달성 가능)
if benefit<=0:
    print(-1)
else:
    if fixed_cost % benefit ==0: # 고정비를 이익으로 나눌때 떨어지면 +1 해야 '이익'이 발생
        print((fixed_cost // benefit)+1)
    else:
        print(math.ceil(fixed_cost /benefit)) # 고정비를 이익으로 나눌때 소수점이 나오면 '올림'
