...
# 하나의 순간이동기가 늘어날때마다 최대 이동 가능 거리는 1, 1-1, 1-2-1, 1-2-2-1, 1-2-3-2-1
# ... 이런식으로 두개씩 동일한 숫자가 늘어난 후에 다음 숫자로 넘어가는 형식임.

# day9_06.py
def move_star(distance):
    count = 0 # 몇 번째 순간이동기인지 저장
    sum = 0 # 최대거리 저장
    i = 1 # 2번씩 동일한 숫자로 사용될 변수 
    while (True):
        sum += i # 최대 이동거리 증가(동일한 숫자 1번째)
        count += 1 # 순간 이동기 갯수 증가
        if sum >= distance: # 최대 이동거리>움직여야 하는 거리인 순간 이동기 갯수 출력
            print(count)
            break
        sum += i # 최대 이동거리 증가(동일한 숫자 2번쨰)
        count += 1 # 순간 이동기 갯수 증가
        if sum >= distance:
            print(count)
            break
        i += 1 # 2번 사용하였으므로 숫자 증가.
...
for _ in range(test_num):
    ...
    sub_distance = b - a # 거리를 저장(b>a로 가정)
    move_star(sub_distance) # 함수 호출
