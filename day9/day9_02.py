...
for _ in range(t):
    ...
    if n%h!=0: #n이 h의 배수가 아니라면, n을 h로 나눈 나머지가 층, n을 h로 나눈 몫에 +1한것이 호수
        print(f'{int(str(n%h).zfill(2)+str(n//h+1).zfill(2))}')
    else: #n이 h의 배수라면, h가 층, n//h로 나눈 몫이 호수수
       print(f'{int(str(h).zfill(2)+str(n//h).zfill(2))}')
